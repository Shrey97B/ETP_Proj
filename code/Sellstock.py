...
class Sellst(Frame):
    ...
    def __init__(self,mw,usern):
        ...
        self.lab1 = Label(self,text='Sell Stock',font=('Helvetica',16))
        ...
        self.numshlab = Label(self.tf,text='Number of shares to sell:')
        ...
        self.sellb = Button(self.tf,text='Sell Stocks',command=self.inssellord,bg='blue')
        self.sellb.place(relx=0.5,rely=0.8)
    ...
    def inssellord(self):
        ...
        shareq = 'select quantity from portfolio where tr_un=:1 and stsymb=:2'
        cur.execute(shareq,(trunam,stsymb))
        res = cur.fetchall()
        if(len(res)==0):
            ownedsh = 0
        else:
        ...
        if ns>ownedsh:
            messagebox.showinfo('','You do not own sufficient shares to sell')
            return
        ...
        newid = cur2.var(cx_Oracle.STRING)

        sql_params = (trunam,stsymb,brunam,'S',ns,'P',brf,newid)
        cur2.execute(insq1, sql_params)
        nid=(newid.getvalue())[0]
        
        dbc = DatabaseCon()
        dbc.createExecuteSellOrderExpJob(self.con,nid)
        print(nid)
        messagebox.showinfo('','Sell Order Request has been placed.\nIt will expire after 5 minutes if not completed')
